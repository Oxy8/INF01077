...
// A variante SIMD é escolhida em tempo de compilação pelo Makefile. O pragma
// fica propositalmente apenas nos loops cuja independência foi revisada.
...
std::array<unsigned char, 2> find_min_and_max_luminance_on_gray_scale_image(int width, int height, unsigned char* data){
    ...
    #pragma omp parallel for schedule(runtime) reduction(min:local_min) reduction(max:local_max)
    for(int j = 0; j < height; j++){
        for(int i = 0; i < width; i++){
            int index = (j * width + i) * 3;
