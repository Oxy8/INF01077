            ...
            for build_index, build in enumerate(builds):
                for layout in layouts:
                    ...
                    if any(value is None for value in values):
                        continue
                    series.append((f"{layout_label(layout)} · {build}", [float(value["Mean_ms"]) for value in values if value], color(build, build_index), [float(value["StdDev_ms"]) for value in values if value]))
                    for thread, value in zip(THREADS, values):
                        extracted.append({"Image": image, "Operation": operation, "Threads": thread, "Build": build, "Layout": layout, "Kernel_ms": value["Mean_ms"], "StdDev_ms": value["StdDev_ms"]})
            if series:
                path = figures / f"01_kernels_{Path(image).stem}_{operation}.svg"
                line_chart(path, f"Kernels — {operation}, {image}", THREADS, series, "Tempo médio (ms)", numeric_x=True)
                links.append((path.name, f"Kernels por threads — {operation}, {image}"))
    write_csv(tables / "kernels_por_layout.csv", ["Image", "Operation", "Threads", "Build", "Layout", "Kernel_ms", "StdDev_ms"], extracted)
    return links
            ...
            for index, build in enumerate(builds):
                for layout in layouts:
                    ...
                    if not any(value is None for value in values):
                        series.append((f"{layout_label(layout)} total · {build}", [float(value["Mean_ms"]) for value in values if value], color(build, index), [float(value["StdDev_ms"]) for value in values if value]))
            if series:
                path = figures / f"02_total_{Path(image).stem}_{operation}.svg"
                line_chart(path, f"Custo total com conversões — {operation}, {image}", THREADS, series, "Tempo médio (ms)", numeric_x=True)
                links.append((path.name, f"Custo total SoA — {operation}, {image}"))
    return links
    ...
    for image in images:
        for operation in operations:
            ...
            for index, build in enumerate(builds):
                ...
                if any(value is None for value in values):
                    continue
                groups.append((build, [float(value["Mean_ms"]) for value in values if value], color(build, index)))
                extracted.extend({"Image": image, "Operation": operation, "Build": build, "Component": label, "Mean_ms": value["Mean_ms"], "StdDev_ms": value["StdDev_ms"]} for label, value in zip(labels, values))
            if groups:
                path = figures / f"03_fases_20t_{Path(image).stem}_{operation}.svg"
                bar_chart(path, f"Componentes em 20 threads — {operation}, {image}", labels, groups, "Tempo médio (ms)")
                links.append((path.name, f"Conversões e passadas em 20 threads — {operation}, {image}"))
    write_csv(tables / "fases_20_threads.csv", ["Image", "Operation", "Build", "Component", "Mean_ms", "StdDev_ms"], extracted)
    return links
...
def simd_heatmaps(rows: list[dict[str, object]], figures: Path, tables: Path) -> list[tuple[str, str]]:
            ...
            for operation in sorted({str(row["Operation"]) for row in rows}):
                for layout in layouts:
                    ...
                    for thread in THREADS:
                        ...
                        values.append(ratio)
                        if base and tested:
                            extracted.append({"Image": image, "Operation": operation, "Layout": layout, "Threads": thread, "Scalar_build": scalar, "SIMD_build": simd, "Scalar_ms": base["Mean_ms"], "SIMD_ms": tested["Mean_ms"], "Speedup_scalar_over_simd": ratio})
    ...
    write_csv(tables / "efeito_simd.csv", ["Image", "Operation", "Layout", "Threads", "Scalar_build", "SIMD_build", "Scalar_ms", "SIMD_ms", "Speedup_scalar_over_simd"], extracted)
    return links
