...
unset OMP_PLACES GOMP_CPU_AFFINITY ONLY_ADAPTIVE

affinity_places_for_threads() {
    case "$1" in
        20) printf 'cores\n' ;;
        40) printf 'threads\n' ;;
        *)
            printf 'Numero de threads sem configuracao de afinidade: %s\n' "$1" >&2
            return 1
            ;;
    esac
}
...
CPU_Model=$cpu_model
Kernel=$kernel
Affinity_Label=spread_20_cores_40_threads
OMP_DYNAMIC=$OMP_DYNAMIC
OMP_PLACES=20:cores,40:threads
...
write_collection_metadata() {
    ...
    local omp_places
    omp_places="$(affinity_places_for_threads "$threads")"
    cat > "$path" <<EOF
Campaign_ID=$campaign_id
...
Affinity_Label=spread_${threads}_${omp_places}
